...
**Status:** **DONE (2026-08-01, web merge `942cb66`)** — все три фазы закрыты, каждая
подтверждена живым прогоном автора.
...
## Как реализовано против спека (читать перед тем, как верить тексту ниже)

Спек писался ДО имплементации; ниже он оставлен как есть — это аудит-трейл решений.
Расхождения:

1. **Порядок фаз изменён автором: B (herdr) сделана первой, вперёд A.** Причина — herdr
   нужен был в бою сразу, а фаза A даёт ноль видимого выигрыша при ненулевом риске на
   пути отправки. Цена (третья копия orca-паттерна) прожила один шаг и схлопнулась в A/C.
2. **`herdr agent prompt` сабмитит сам** — вопрос, помеченный ниже как «ЕДИНСТВЕННОЕ
   НЕПРОВЕРЕННОЕ», **закрыт**: подтверждено живым прогоном и release-notes 0.7.5, где
   `prompt` назван atomic. `agent send-keys` в permission не понадобился.
3. **`isAvailable()` из контракта провайдера выкинут.** Это был бы второй вызов ради того
   же ответа: пикер прячет секцию по отклонённому промису `listTargets()`.
4. **`send()` возвращает строку «куда ушло»**, а не `void` — тост формулируется в одном
   месте, а не тремя расходящимися копиями в провайдерах.
5. **`TerminalTarget` получил поле `meta`** (правая колонка: pane id / каталог / тип агента).
6. **Добавлена `sameBinding()`** в реестр — преселект пикера сравнивает стабильные
   дескрипторы, а не живые хендлы.
...
- **НЕ унифицировать механику отправки.** Абстракция общая, но tmux шлёт через
  `paste-buffer -p` + settle, Orca — ручной bracketed-paste + settle, herdr — atomic
  `agent prompt` без обёртки и без settle. Подгонять herdr под общий шаблон нельзя:
  `\x1b[200~` уедет в промпт литералом, а `--json` уронит команду.
