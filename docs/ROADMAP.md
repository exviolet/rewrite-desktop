...
- **OSS-публикация механика** — курс принят 2026-07-07 (staged A→B→C, см. секцию). **Закрыто:** лицензия MIT, release-механика (`v0.1.0` выпущен 2026-08-09, prebuilt AppImage из контейнера, `linuxdeploy` починен). **Осталось:** формат README-демо (GIF/asciinema), площадки стадии C, нужен ли CI (решать со второго релиза).
