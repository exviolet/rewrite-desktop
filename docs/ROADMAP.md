...
| 12 | Tab grouping (именованные цветные группы внутри workspace) | [tasks/14-tab-grouping.md](../tasks/14-tab-grouping.md) | **done** (2026-07-27) — [#4](https://github.com/exviolet/rewrite-desktop/issues/4) распаркован: условие reviv'а выполнено (боль осталась ВНУТРИ одного workspace после 2+ недель с изоляцией), плюс настойчивый запрос 2-го юзера. **2026-08-01:** палитра расширена 6 → 12 цветов (снова запрос 2-го юзера) — аддитивно, первые шесть и их порядок не тронуты, иначе перекрасились бы существующие группы |
...
**Опрос, а не подписка** — вынужденно: хуков от herdr к Rewrite не ведёт, а завести свои значило
...
IndexedDB автора), поднялся, создал собственный `com.rewrite.app/databases`, прожил 14 с,
...
<https://github.com/exviolet/rewrite-desktop/releases/tag/v0.1.0> — первый prebuilt-артефакт.
...
**asciinema отпадает физически, а не по вкусу.** Он пишет поток терминала, а Rewrite — GTK-окно
...
зритель видит, что текст сам собой перелетел в терминал. Окно вынесено за рамку Rewrite, иначе
...
1. **`app_id` у двух сборок разный:** AppImage выставляет `Rewrite-desktop` (с заглавной),
   сборка из исходников — `rewrite-desktop`. Счётчик окон по точному совпадению даёт ноль при
   живом окне. Важно и практически: правила окон в композиторе, написанные под одну сборку, не
   сработают для другой.
2. **`kill` по pid обёртки AppImage не убивает приложение** — внутренний `rewrite-desktop`
...
заметок, markdown там конечный продукт; в Rewrite markdown — леса вокруг промпта, который
...
не рендерится ничем, а «важно» дешевле сказать словом. Автор подтвердил: в Rewrite пишутся
...
Чинится дёшево (второй аккорд без nav-клавиш + правка `reorderTab`), но **вынесено отдельной
задачей** по решению автора: не разменивать заход на две темы.
...
- **OSS-публикация механика** — курс принят 2026-07-07 (staged A→B→C, см. секцию). **Закрыто:** лицензия MIT, release-механика (`v0.1.0` выпущен 2026-08-09, prebuilt AppImage из контейнера, `linuxdeploy` починен), **формат демо** (2026-08-11, см. ниже). **Осталось:** площадки стадии C, нужен ли CI (решать со второго релиза).
