...
## Context
- **Solo dev, ранняя стадия (v0.1.1), 2 пользователя** (автор + друг — начал пользоваться сам, 2026-07). Курс на staged OSS. Миграции данных, feature flags, backwards-compat shims **не нужны** — clean sweeps допустимы, но разрушающие data-изменения (IndexedDB) координировать с другом (у него накоплены данные).
