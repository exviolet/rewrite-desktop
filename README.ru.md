...
- Интеграция с [Herdr](https://herdr.dev) — то же самое для панели Herdr-агента.
  ...
  переживает рестарт сервера и ребут.
...
- Работает ровно один инстанс: повторный запуск не поднимает второе окно, а
  фокусирует уже открытое. Две копии на одной базе тихо съедали бы работу друг
  друга — запись идёт снапшотом целиком, поэтому отставший инстанс затирает то,
  что успел создать первый.
